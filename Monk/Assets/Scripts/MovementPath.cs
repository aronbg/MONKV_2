...
{ //ik heb deze code een keer overgeschreven en begrijp hem compleet
    // nu heb ik besloten om de cleanste versie te pakken en deze verder uit te werken naar mijn situatie
    #region Enums
    ...
    private bool playerisnearthebird = true;
    ...
    void Update()
    {
        playerisnearthebird = Physics.CheckSphere(PathSequence[movingTo].position, 60f, playerlayer);
    }
...
//*********************************PAUSES HERE******************************************************//
            ...
            {
                if (movingTo <= 0 && playerisnearthebird)
                {
                    movementDirection = movementDirection+ 1; //Seting to 1 moves forward
                
                }
                if(playerisnearthebird == false){
                    movementDirection = 0;
                }
